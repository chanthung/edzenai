        ...
        <TabsContent value="school" className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} disabled={isRestricted} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="address">Address</Label>
                  <Input id="address" placeholder="123 School Street, City" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} disabled={isRestricted} />
                </div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="phone"><Phone className="h-4 w-4 inline mr-1" />Phone Number</Label>
                  <Input id="phone" placeholder="+91 98765 43210" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} disabled={isRestricted} />
                </div>
                <div className="space-y-2">
          ...
          <Card className="card-elevated">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="upi_id" placeholder="school@upi" value={formData.upi_id} onChange={(e) => setFormData({ ...formData, upi_id: e.target.value })} disabled={isRestricted} />
                <p className="text-sm text-muted-foreground">This will be shown to parents for making payments via GPay, PhonePe, etc.</p>
              </div>
              <RestrictedOverlay isRestricted={isRestricted}>
                <div className="space-y-2">
                  <Label>Payment QR Code</Label>
                  <input ref={fileInputRef} type="file" accept="image/*" onChange={handleQrUpload} className="hidden" disabled={isRestricted} />
                  {formData.qr_code_url ? (
                    <div className="p-4 bg-muted/50 rounded-lg space-y-3">
                      <p className="text-sm font-medium">QR Code Preview</p>
                      <img src={formData.qr_code_url} alt="Payment QR Code" className="max-w-[200px] rounded-lg border" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      <div className="flex gap-2">
                        <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} disabled={isUploadingQr || isRestricted}>
                          {isUploadingQr ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Upload className="h-4 w-4 mr-2" />} Replace
                        </Button>
                        <Button type="button" variant="outline" size="sm" onClick={handleRemoveQr} className="text-destructive hover:text-destructive" disabled={isRestricted}>
                          <Trash2 className="h-4 w-4 mr-2" /> Remove
                        </Button>
                      </div>
                    </div>
                  ) : (
                    <div className={`border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center transition-colors ${isRestricted ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-primary/50'}`} onClick={() => !isRestricted && fileInputRef.current?.click()}>
                      {isUploadingQr ? <Loader2 className="h-8 w-8 mx-auto mb-2 animate-spin text-muted-foreground" /> : <Upload className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />}
          ...
          <Card className="card-elevated">
            ...
            <CardContent className="space-y-4">
              ...
              <RestrictedButton isRestricted={isRestricted}>
                <Button onClick={handleChangePassword} disabled={isChangingPassword || !passwordData.newPassword || !passwordData.confirmPassword || isRestricted} variant="destructive">
        ...
        <TabsContent value="templates" className="space-y-6">
          {editingTemplate !== undefined ? (
            <TemplateEditor
              ...
              onBack={() => setEditingTemplate(undefined)}
            />
          ) : (
            <>
              <TemplateList
