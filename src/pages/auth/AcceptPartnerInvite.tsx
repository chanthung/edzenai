...
import { Loader2, CheckCircle, XCircle, Mail, Handshake } from "lucide-react";

type InviteState =
  | { status: "loading" }
  | { status: "valid"; email: string; name: string; referralCode?: string; commissionPercent?: number }
...
export default function AcceptPartnerInvite() {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(
          `https://${projectId}.supabase.co/functions/v1/accept-partner-invite?token=${encodeURIComponent(token)}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const projectId = import.meta.env.VITE_SUPABASE_PROJECT_ID;
      const res = await fetch(
        `https://${projectId}.supabase.co/functions/v1/accept-partner-invite`,
        {
          method: "POST",
          headers: {
    ...
    <div className="min-h-[100dvh] flex items-start pt-8 sm:items-center sm:pt-0 justify-center bg-background px-4 overflow-y-auto">
      <div className="w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold">EdZen AI Partner Program</h1>
        </div>
        ...
        <Card className="border-border/50 shadow-card">
          ...
          {invite.status === "invalid" && (
            <>
              <CardHeader className="text-center">
                ...
                <CardDescription>
                  {invite.reason === "expired" && "This invite has expired. Please contact the EdZen AI team for a new one."}
                  {invite.reason === "used" && "This invite has already been accepted. Please sign in instead."}
                  {invite.reason === "invalid" && "This invite link is invalid."}
          ...
          {invite.status === "valid" && !success && (
            <>
              <CardHeader className="text-center">
                ...
                <CardDescription>
                  You've been invited to join the <strong>EdZen AI Partner Program</strong>.
                  {invite.commissionPercent != null && (
                    <> You'll earn <strong>{invite.commissionPercent}%</strong> commission on every payment from referred schools.</>
                  )}
                  <br />Set a password to activate your partner account.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {invite.referralCode && (
                    <div className="space-y-2">
                      ...
                      <Input value={invite.referralCode} readOnly disabled className="bg-muted/50 font-mono" />
                    </div>
                  )}
          ...
          {success && (
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">Taking you to your partner dashboard…</p>
            </CardContent>
          )}
        </Card>
      </div>
